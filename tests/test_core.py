...
def test___create_json_request_content():
    assert _core.create_json_request_content("{}", {"param1": "value1"}) == _json.dumps({})
    ...
    result = _core.create_json_request_content(content_structure, params)
    assert result == expected_result
    result2 = _core.create_json_request_content(content_structure, params2)
    assert result2 != expected_result

    with _pytest.raises(Exception):
        _core.create_json_request_content(None, None)
    with _pytest.raises(Exception):
        _core.create_json_request_content(None, {})
    with _pytest.raises(Exception):
        _core.create_json_request_content("", None)
    with _pytest.raises(Exception):
        _core.create_json_request_content("", {})
