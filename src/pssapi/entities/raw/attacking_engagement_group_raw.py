...
class AttackingEngagementGroupRaw(_EntityBaseRaw):
    XML_NODE_NAME: str = "AttackingEngagementGroup"

    def __init__(self, attacking_engagement_group_info: _EntityInfo) -> None:
        self._dict: _Dict[str, _Any] = {}
        self._engagement_group_id: int = _parse.pss_int(attacking_engagement_group_info.pop("EngagementGroupId", None))
        self._engagement_group_icon_sprite_id: int = _parse.pss_int(attacking_engagement_group_info.pop("EngagementGroupIconSpriteId", None))
        self._engagement_group_name: str = _parse.pss_str(attacking_engagement_group_info.pop("EngagementGroupName", None))
        self._engagement_group_users: _List[_entities.EngagementGroupUser] = (
            [_entities.EngagementGroupUser(child_info) for child_info in attacking_engagement_group_info.pop("EngagementGroupUsers")[0].get("EngagementGroupUser", [])]
            if attacking_engagement_group_info.get("EngagementGroupUsers")
            else []
        )
        self._engagement_id: int = _parse.pss_int(attacking_engagement_group_info.pop("EngagementId", None))
        self._power_limit: int = _parse.pss_int(attacking_engagement_group_info.pop("PowerLimit", None))
        self._requires_approval: bool = _parse.pss_bool(attacking_engagement_group_info.pop("RequiresApproval", None))
        self._update_date: _datetime = _parse.pss_datetime(attacking_engagement_group_info.pop("UpdateDate", None))
        self._user_limit: int = _parse.pss_int(attacking_engagement_group_info.pop("UserLimit", None))
