...
async def get_catalog_quantity(production_server: str, **params) -> _GetCatalogQuantity:
    ...
    result = await _core.get_entities_from_path(
        ((_GetCatalogQuantity, "GetCatalogQuantity", False),), "LiveOpsService", production_server, GET_CATALOG_QUANTITY_BASE_PATH, "GET", response_gzipped=False, **params
    )
    return result
...
async def get_today_live_ops(production_server: str, device_type: str, language_key: str, **params) -> _LiveOps:
    ...
    result = await _core.get_entities_from_path(((_LiveOps, "LiveOps", False),), "GetTodayLiveOps", production_server, GET_TODAY_LIVE_OPS_BASE_PATH, "GET", response_gzipped=False, **params)
    return result


async def get_today_live_ops_2(production_server: str, device_type: str, language_key: str, **params) -> _LiveOps:
    params = {"deviceType": device_type, "languageKey": language_key, **params}
    result = await _core.get_entities_from_path(((_LiveOps, "LiveOps", False),), "GetTodayLiveOps", production_server, GET_TODAY_LIVE_OPS_2_BASE_PATH, "GET", response_gzipped=False, **params)
    return result
