...
class ChallengeService(_ServiceBase):
    async def list_all_challenge_designs(self, design_version: int = None) -> _List[_ChallengeDesign]:
        result = await _ChallengeServiceRaw.list_all_challenge_designs_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result
