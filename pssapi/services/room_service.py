...
class RoomService(_ServiceBase):
    async def list_action_types(self, design_version: int = None) -> _List[_ActionType]:
        result = await _RoomServiceRaw.list_action_types_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result

    async def list_condition_types(self, design_version: int = None) -> _List[_ConditionType]:
        result = await _RoomServiceRaw.list_condition_types_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result

    async def list_craft_designs(self, design_version: int = None) -> _List[_CraftDesign]:
        result = await _RoomServiceRaw.list_craft_designs(production_server=self.production_server, design_version=design_version)
        return result

    async def list_missile_designs(self, design_version: int = None) -> _List[_MissileDesign]:
        result = await _RoomServiceRaw.list_missile_designs(production_server=self.production_server, design_version=design_version)
        return result

    async def list_room_design_purchase(self, design_version: int = None) -> _List[_RoomDesignPurchase]:
        result = await _RoomServiceRaw.list_room_design_purchase(production_server=self.production_server, design_version=design_version)
        return result

    async def list_room_designs(self, design_version: int = None) -> _List[_RoomDesign]:
        result = await _RoomServiceRaw.list_room_designs_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result
