...
class ItemService(_ServiceBase):
    async def list_item_design_actions(self, design_version: int = None) -> _List[_ItemDesignAction]:
        result = await _ItemServiceRaw.list_item_design_actions(production_server=self.production_server, design_version=design_version)
        return result

    async def list_item_designs(self, design_version: int = None) -> _List[_ItemDesign]:
        result = await _ItemServiceRaw.list_item_designs_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result
