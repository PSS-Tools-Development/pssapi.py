...
class CharacterService(_ServiceBase):
    async def list_all_character_design_actions(self, design_version: int = None) -> _List[_CharacterDesignAction]:
        result = await _CharacterServiceRaw.list_all_character_design_actions(production_server=self.production_server, design_version=design_version)
        return result

    async def list_all_character_designs(self, design_version: int = None) -> _List[_CharacterDesign]:
        result = await _CharacterServiceRaw.list_all_character_designs_2(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result

    async def list_all_draw_designs(self, design_version: int = None) -> _List[_DrawDesign]:
        result = await _CharacterServiceRaw.list_all_draw_designs(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result
