...
class GalaxyService(_ServiceBase):
    async def go_to(self, star_system_id: int, client_date_time: str, checksum: str, access_token: str) -> _List[_Ship]:
        result = await _GalaxyServiceRaw.go_to(self.production_server, star_system_id, client_date_time, checksum, access_token)
        return result

    async def list_marker_generator_designs(self, design_version: int = None) -> _List[_StarSystemMarkerGenerator]:
        result = await _GalaxyServiceRaw.list_marker_generator_designs(self.production_server, self.language_key, design_version)
        return result
    ...
    async def list_star_systems(self, design_version: int = None) -> _List[_StarSystem]:
        result = await _GalaxyServiceRaw.list_star_systems(self.production_server, self.language_key, design_version)
        return result
