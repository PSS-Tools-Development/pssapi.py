...
class MissionService(_ServiceBase):
    async def list_all_mission_designs(self, design_version: int = None) -> _List[_MissionDesign]:
        result = await _MissionServiceRaw.list_all_mission_designs_4(production_server=self.production_server, design_version=design_version, language_key=self.language_key)
        return result
