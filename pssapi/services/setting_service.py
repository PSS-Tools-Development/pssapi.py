...
class SettingService(_ServiceBase):
    async def get_latest_version(self, device_type: str) -> _List[_Setting]:
        result = await _SettingServiceRaw.get_latest_version_3((await self.get_production_server()), device_type, self.language_key)
        return result

    async def list_all_news_designs(self, design_version: int = None) -> _List[_NewsDesign]:
        result = await _SettingServiceRaw.list_all_news_designs((await self.get_production_server()), design_version, self.language_key)
        return result
