...
class FileRaw:
    ...
    def __init__(self, file_info: _EntityInfo) -> None:
        ...
        self.date_updated: datetime = _parse.pss_datetime(
            file_info.get('DateUpdated'))
        self.file_download_category: str = _parse.pss_str(
            file_info.get('FileDownloadCategory'))
        self.filename: str = _parse.pss_str(file_info.get('Filename'))
        self.id: int = _parse.pss_int(file_info.get('Id'))
        self.is_language_specific: bool = _parse.pss_bool(
