...
class CraftDesignRaw():
    ...
    def __init__(self, craft_design_info: _EntityInfo) -> None:
        self.__craft_design_id: int = _parse.pss_int(craft_design_info.get('CraftDesignId'))
        self.__craft_name: str = _parse.pss_str(craft_design_info.get('CraftName'))
        self.__flight_speed: int = _parse.pss_int(craft_design_info.get('FlightSpeed'))
        self.__reload: int = _parse.pss_int(craft_design_info.get('Reload'))
        self.__sprite_id: int = _parse.pss_int(craft_design_info.get('SpriteId'))
        self.__missile_design_id: int = _parse.pss_int(craft_design_info.get('MissileDesignId'))
        self.__volley: int = _parse.pss_int(craft_design_info.get('Volley'))
        self.__volley_delay: int = _parse.pss_int(craft_design_info.get('VolleyDelay'))
