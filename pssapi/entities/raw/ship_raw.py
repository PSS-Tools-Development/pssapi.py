...
class ShipRaw:
    ...
    def __init__(self, ship_info: _EntityInfo) -> None:
        ...
        self.immunity_date: _datetime = _parse.pss_datetime(ship_info.get('ImmunityDate'))
        self.items: _List[_Item] = [_Item(child_info) for child_info in ship_info.get('Items')] if ship_info.get('Items') else []
        ...
        self.rooms: _List[_Room] = [_Room(child_info) for child_info in ship_info.get('Rooms')] if ship_info.get('Rooms') else []
        ...
        self.star_system_arrival_date: _datetime = _parse.pss_datetime(ship_info.get('StarSystemArrivalDate'))
        self.star_system_id: int = _parse.pss_int(ship_info.get('StarSystemId'))
        self.status_start_date: _datetime = _parse.pss_datetime(ship_info.get('StatusStartDate'))
        ...
        self.update_date: _datetime = _parse.pss_datetime(ship_info.get('UpdateDate'))
        self.upgrade_ship_design_id: int = _parse.pss_int(ship_info.get('UpgradeShipDesignId'))
        self.upgrade_start_date: _datetime = _parse.pss_datetime(ship_info.get('UpgradeStartDate'))
        self.user_id: int = _parse.pss_int(ship_info.get('UserId'))
        self.user_star_systems: _List[_UserStarSystem] = [_UserStarSystem(child_info) for child_info in ship_info.get('UserStarSystems')] if ship_info.get('UserStarSystems') else []
